from re import L
from src.intercaatmaster.intercaatWrapper import intercaat
from halo import Halo

@Halo(text='Loading', spinner='dots')
def intercaatWT(pdb_file, qc, ic):
    return intercaat(pdb_file, qc, ic, fp="input/")
...
def intercaatMutant(position, pdb_file, qc, ic, fp, intercaat_wt_scores, wt_AA, mutant_AA ):
    ...
    results = intercaat(pdb_file,qc,ic,mi=0,fp= fp)
    if key in results:
        interactions = results[key][2] 
    else:
        interactions = 0 
    interactions = interactions/avergae_interactions[mutant_AA]
    wt_interactions = intercaat_wt_scores[position] / avergae_interactions[wt_AA]
    dinteractions  = wt_interactions - interactions
    return round(dinteractions, 3)
